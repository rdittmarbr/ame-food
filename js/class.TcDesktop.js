...
TcDesktop.prototype.setURL = function (aURL) {
  ...
  window.history.pushState("object or string", "Title", this.FURLA + aURL);
  return true;
};
/*------------------=-----------=------------------------------------------------------------------------------
    ...
    if (tag.type && ",text,password".search(tag.type) > 0) {
      ...
      if (tag.parentNode.nodeName == "DIV") {
        ...
        if (tag.labels.length > 0) {
          ...
          if (tag.value != "") {
            tag.parentNode.addClass = "focused";
            tag.addClass = "filled";
          }
...
TcDesktop.prototype.sendForm = function (o) {
  ...
  for (i = o.elements.length - 1; i >= 0; i = i - 1) {
    ...
    if (input.name) {
      ...
      if (aName == input.name) {
        if (this.FDebug)
          this.debug("Erro ao processar formulário : " + input.name, 2);
        clearChild(body);
        body.innerHTML =
          '<div class="FATALERROR">ERRO AO PROCESSAR FORMULARIO.</div>';
        return false;
      }
      ...
      switch (input.nodeName) {
        case "INPUT":
          switch (input.type) {
            ...
            case "reset":
            case "submit":
              aParam["V"].push(jsonData(aName, input.value));
              break;
            case "checkbox":
            case "radio":
              if (input.checked) {
...
TcDesktop.prototype.processModule = function (aModule, aData) {
  ...
  if (aData.js && aData.js.before) {
    ...
    for (i in aData.js.before) {
      if (this.FDebug)
        this.debug("Executando comando JS " + aData.js.before[i]);
    ...
    for (h in aData.main) {
      ...
      if (o && aData.main[h]) {
        ...
        if (this.FAnimated) {
          ...
          if (this.FDebug && i.lenrpcServergth > 0)
            this.debug("atualizando a Animação do Input");
  ...
  if (aData.data) {
    for (h in aData.data) {
      ...
      if (o) {
        ...
        if (this.FDebug)
          this.debug("in_" + aModule + "_" + h + "=" + aData.data[h]);
      } else {
        if (this.FDebug)
          this.error("in_" + aModule + "_" + h + "=" + aData.data[h]);
      }
    }
  }
...
TcDesktop.prototype.rpcServer = function (aParam) {
  ...
  for (h in aParam) ajax.addParam(h, aParam[h]);
  ...
  ajax.FProcess = function (ajax_request) {
    ...
    if (ajax_request.js && ajax_request.js.before) {
      for (h in ajax_request.js.before)
        if (
          (ajax_request.js.before[h] = eval(ajax_request.js.before[h])) &&
          isFunction(ajax_request.js.before[h])
        )
    ...
    if (ajax_request.js && ajax_request.js.after)
      for (h in ajax_request.js.after)
        if (
          (ajax_request.js.after[h] = eval(ajax_request.js.after[h])) &&
          isFunction(ajax_request.js.after[h])
        )
